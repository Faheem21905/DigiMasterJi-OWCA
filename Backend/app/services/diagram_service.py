...
class DiagramService:
    ...
    async def generate_diagram_with_llm(
        ...
        Args:
            ...
            diagram_type: Type of diagram suggested
            
        Returns:
            Dictionary with diagram data or None if generation fails
        """
        try:
...
OUTPUT: Return ONLY the complete SVG code. No explanation, no markdown, no code blocks. Just the raw SVG starting with <svg and ending with </svg>."""
            ...
            if not result.get("success"):
                logger.warning(f"[DIAGRAM LLM] LLM generation failed: {result.get('error')}")
                return None
            ...
            logger.info(f"[DIAGRAM LLM] Successfully generated SVG ({len(svg_content)} bytes)")
    ...
    def generate_diagram(
        ...
        """
        Generate a diagram for the response if appropriate (sync version).
