 ...
 * 
 * Insights are automatically generated after each quiz completion and stored
 * in the user's profile for quick access
 */
export default function LearningInsightsPage() {
  ...
  // Fetch stored insights on mount (auto-generated after quiz completion)
  useEffect(() => {
  ...
  // First try to get stored insights (faster), fall back to generating new ones
  const fetchStoredInsights = async () => {
    try {
      ...
      // If no stored insights, fall back to generating new ones
  ...
  const fetchInsights = async () => {
    try {
      ...
      const response = await quizzesApi.getLearningInsights(30);
      setInsights(response.data);
  ...
  // Refresh insights - regenerate fresh analysis
  const handleRefresh = async () => {
    setRefreshing(true);
    try {
      const response = await quizzesApi.refreshInsights();
      setInsights(response.data);
      setGeneratedAt(new Date());
      setIsAutoGenerated(false);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#050816] overflow-y-auto">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 py-6 pb-24">
        ...
        {/* Language Toggle and Auto-Generated Indicator */}
        <motion.div
