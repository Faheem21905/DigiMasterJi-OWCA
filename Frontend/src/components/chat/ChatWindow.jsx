...
export default function ChatWindow({
  ...
  if (!conversation) {
    return (
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8">
          ...
          >
            <motion.div
              ...
              className="w-24 h-24 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-orange-500/20 to-cyan-500/20 flex items-center justify-center border border-white/[0.08]"
            >
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {['Photosynthesis', 'Algebra', 'Physics Laws', 'Chemistry'].map((topic, i) => (
                <motion.button
                  key={topic}
                  ...
                  className="px-4 py-2 rounded-full bg-white/[0.05] border border-white/[0.08] text-white/60 text-sm hover:bg-white/[0.1] hover:text-white hover:border-orange-500/30 transition-all cursor-pointer"
                >
                  {topic}
                </motion.button>
              ))}
        ...
        <div className="flex-shrink-0 p-4 sm:p-6 border-t border-white/[0.05]">
          <ChatInputWithVoice
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden">
      {/* Chat Header */}
      <div className="flex-shrink-0 flex items-center justify-between px-4 sm:px-6 py-4 border-b border-white/[0.05] bg-white/[0.02] backdrop-blur-xl">
        ...
        {messages.length === 0 && !isTyping && !isLoading && (
          ...
          >
            <motion.div
              ...
              className="w-16 h-16 rounded-xl bg-gradient-to-br from-orange-500 to-amber-600 flex items-center justify-center mb-4 shadow-lg shadow-orange-500/30"
            >
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className={`flex gap-3 ${i % 2 === 0 ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex-1 max-w-[70%] space-y-2 ${i % 2 === 0 ? 'items-end' : 'items-start'}`}>
                  <div className="h-4 bg-white/[0.05] rounded-lg animate-pulse" style={{ width: `${60 + Math.random() * 30}%` }} />
                  <div className="h-4 bg-white/[0.05] rounded-lg animate-pulse" style={{ width: `${40 + Math.random() * 40}%` }} />
                </div>
              </div>
            ))}
          </div>
      ...
      <AnimatePresence>
        {showScrollButton && (
          <motion.button
            ...
            className="absolute bottom-24 right-6 p-3 rounded-full bg-orange-500 text-white shadow-lg shadow-orange-500/30 hover:bg-orange-400 transition-colors"
          >
